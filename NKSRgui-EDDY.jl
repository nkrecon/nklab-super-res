...
#perform eddy correction using singularity image
#To overwrite change last param to 1
print("Performing Eddy Correction on ",filenameA,"\n")
run(`./runEddyRobust.sh $SINGIMG $dirA $dirOut $fileA eddyA 1`)
fileA_eddy="$dirOut/eddyA.nii.gz"

print("Performing Eddy Correction on ",filenameB,"\n")
run(`./runEddyRobust.sh $SINGIMG $dirB $dirOut $fileB eddyB 2`)
fileB_eddy="$dirOut/eddyB.nii.gz"

print("Performing Eddy Correction on ",filenameC,"\n")
run(`./runEddyRobust.sh $SINGIMG $dirC $dirOut $fileC eddyC 3`)
...
print("Creating and Saving intermediate imag as input to eddy correct\n")
headerinfo = deepcopy(headerinfoA)
#this is a lazy way to fix the fact that EDDY creates a float as output
headerinfo["datatype"]= convert(Array{Int16},[4])
headerinfo["bitpix"]= convert(Array{Int16},[16])
...
#using eddy correct here because of problems with the NIFTI
...
#run(`./runEddyRobust.sh $SINGIMG $dirOut $dirOut input_of_eddy_correct.nii output_of_eddy_correct sr`)
...
print("...Now...Cleaning up temporary files\n")
  try
    run(`./rmFiles.sh $dirOut/eddyA.e*`)
    run(`./rmFiles.sh $dirOut/eddyB.e*`)
    run(`./rmFiles.sh $dirOut/eddyC.e*`)
    run(`./rmFiles.sh $dirOut/input_of_eddy_correct*`)
    run(`./rmFiles.sh $dirOut/output_of_eddy_correct*`)
  catch
